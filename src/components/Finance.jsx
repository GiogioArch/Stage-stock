    ...
    <div>
      ...
      <GradientHeader
        ...
        stats={[
          ...
          { value: `${Math.round(margin)}€`, label: 'Marge' },
        ]}
      />
        ...
        <div>
          ...
          <div className="card" style={{ padding: '14px 16px', marginBottom: 12 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div>
                <div style={{ fontSize: 10, fontWeight: 700, color: BASE.textDisabled }}>Ventes réalisées</div>
                <div style={{ fontSize: 20, fontWeight: 600, color: SEMANTIC.success }}>{revenueData.ventesReelles}</div>
              </div>
              <div>
                <div style={{ fontSize: 10, fontWeight: 700, color: BASE.textDisabled }}>CA réalisé</div>
                <div style={{ fontSize: 20, fontWeight: 600, color: SEMANTIC.success }}>{revenueData.caReel}€</div>
              </div>
              <div>
                <div style={{ fontSize: 10, fontWeight: 700, color: BASE.textDisabled }}>Ventes prévues</div>
                <div style={{ fontSize: 20, fontWeight: 600, color: SEMANTIC.warning }}>{revenueData.ventesPrevues}</div>
              </div>
              <div>
                <div style={{ fontSize: 10, fontWeight: 700, color: BASE.textDisabled }}>CA prévu</div>
                <div style={{ fontSize: 20, fontWeight: 600, color: SEMANTIC.warning }}>{revenueData.caPrevu}€</div>
              </div>
            </div>
          </div>
          ...
          {depreciationItems.length > 0 && (
            <div className="card" style={{ padding: '14px 16px', marginBottom: 12 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, textAlign: 'center' }}>
                <div>
                  <div style={{ fontSize: 18, fontWeight: 600, color: SEMANTIC.warning }}>{Math.round(totalBrut)}€</div>
                  <div style={{ fontSize: 9, color: BASE.textMuted, fontWeight: 600 }}>Valeur brute</div>
                </div>
                <div>
                  <div style={{ fontSize: 18, fontWeight: 600, color: SEMANTIC.warning }}>{Math.round(totalAmorti)}€</div>
                  <div style={{ fontSize: 9, color: BASE.textMuted, fontWeight: 600 }}>Amorti</div>
                </div>
                <div>
                  <div style={{ fontSize: 18, fontWeight: 600, color: SEMANTIC.success }}>{Math.round(totalNet)}€</div>
                  <div style={{ fontSize: 9, color: BASE.textMuted, fontWeight: 600 }}>Valeur nette</div>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="card" style={{ padding: '14px 16px' }}>
            ...
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 28, fontWeight: 600, color: SEMANTIC.warning }}>{Math.round(stockValue)}€</div>
          ...
          {/* Legal note */}
          <div style={{
            marginTop: 14, padding: '10px 14px', borderRadius: RADIUS.lg,
            background: `${BASE.bgHover}40`, fontSize: 10, color: BASE.textMuted, lineHeight: 1.6,
          }}>
      ...
      {section === 'revenue' && (
        <div>
          ...
          {revenueData.past.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {revenueData.past.map(ev => (
                <div key={ev.id} className="card" style={{ padding: '12px 14px', borderLeft: `4px solid ${SEMANTIC.success}` }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontSize: 16, fontWeight: 600, color: SEMANTIC.success }}>{ev.ca_reel}€</div>
                      <div style={{ fontSize: 10, color: BASE.textMuted }}>{ev.ventes_reelles} ventes</div>
                    </div>
                  </div>
                  {ev.ca_prevu && (
                    <div style={{ fontSize: 10, color: BASE.textDisabled }}>
          ...
          {revenueData.upcoming.filter(e => e.ca_prevu).length > 0 && (
            <>
              <div style={{ fontSize: 12, fontWeight: 600, color: BASE.textMuted, textTransform: 'uppercase', letterSpacing: 1, margin: '16px 0 10px' }}>
                Prévisions
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {revenueData.upcoming.filter(e => e.ca_prevu).map(ev => (
                  <div key={ev.id} className="card" style={{ padding: '12px 14px', borderLeft: `4px solid ${SEMANTIC.warning}` }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <div style={{ textAlign: 'right' }}>
                        <div style={{ fontSize: 16, fontWeight: 600, color: SEMANTIC.warning }}>{ev.ca_prevu}€</div>
                        {ev.ventes_prevues && <div style={{ fontSize: 10, color: BASE.textMuted }}>{ev.ventes_prevues} ventes</div>}
                      </div>
                    </div>
                  </div>
                ))}
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 12 }}>
            <button onClick={() => setShowAddExpense(!showAddExpense)} style={{
              padding: '8px 16px', borderRadius: RADIUS.md, fontSize: 12, fontWeight: 600,
              background: showAddExpense ? BASE.bgHover : SEMANTIC.danger, color: showAddExpense ? BASE.textMuted : BASE.white,
                ...
                return (
                  <div key={cat} style={{ marginBottom: 8 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 3 }}>
                      <span style={{ color: BASE.text, fontWeight: 600 }}>{EXPENSE_CATS[cat] || cat}</span>
                      <span style={{ fontWeight: 600, color: SEMANTIC.danger }}>{Math.round(data.total)}€</span>
                    </div>
                    <div style={{ height: 4, borderRadius: 2, background: BASE.bgHover, overflow: 'hidden' }}>
                      <div style={{ width: `${pct}%`, height: '100%', borderRadius: 2, background: SEMANTIC.danger }} />
                    </div>
                  </div>
                )
              })}
          ...
          {expenseData.items.length === 0 ? (
            <div className="card" style={{ padding: 32, textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 14, fontWeight: 600, color: BASE.text }}>Aucune dépense</div>
            </div>
          ) : (
              ...
              {expenseData.items.sort((a, b) => (b.date || '').localeCompare(a.date || '')).map((exp, i) => {
                ...
                return (
                  <div key={exp.id} style={{
                    ...
                    borderBottom: i < expenseData.items.length - 1 ? `1px solid ${BASE.bgHover}` : 'none',
                  }}>
                    ...
                    <div style={{ fontSize: 14, fontWeight: 600, color: SEMANTIC.danger }}>{exp.amount}€</div>
                  </div>
                )
              })}
            </div>
      ...
      {section === 'bilan' && (
        <div>
          {(events || []).filter(e => e.date < today).length === 0 ? (
            <div className="card" style={{ padding: 32, textAlign: 'center' }}>
              <div style={{ fontSize: 40, marginBottom: 8 }}></div>
              <div style={{ fontSize: 14, fontWeight: 600, color: BASE.text }}>Aucun concert passé</div>
              <div style={{ fontSize: 12, color: BASE.textMuted, marginTop: 4 }}>Le bilan apparaîtra après les premiers concerts</div>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {(events || []).filter(e => e.date < today).sort((a, b) => b.date.localeCompare(a.date)).map(ev => {
                ...
                return (
                  <div key={ev.id} className="card" style={{
                    padding: '14px 16px', borderLeft: `4px solid ${evMargin >= 0 ? SEMANTIC.success : SEMANTIC.danger}`,
                  }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                      ...
                      <div style={{ textAlign: 'right' }}>
                        <div style={{ fontSize: 18, fontWeight: 600, color: evMargin >= 0 ? SEMANTIC.success : SEMANTIC.danger }}>
                          {evMargin >= 0 ? '+' : ''}{Math.round(evMargin)}€
                        </div>
                        <div style={{ fontSize: 9, color: BASE.textMuted }}>marge</div>
                      </div>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6, fontSize: 11 }}>
                      <div><span style={{ color: BASE.textMuted }}>Merch :</span> <strong style={{ color: SEMANTIC.success }}>{evRevenu}€</strong></div>
                      <div><span style={{ color: BASE.textMuted }}>Billets :</span> <strong>{evTickets}€</strong></div>
                      <div><span style={{ color: BASE.textMuted }}>Sponsors :</span> <strong>{evSponsors}€</strong></div>
                    </div>
                    {evExpenses > 0 && (
                      <div style={{ fontSize: 11, marginTop: 4 }}>
                        <span style={{ color: BASE.textMuted }}>Dépenses :</span> <strong style={{ color: SEMANTIC.danger }}>-{Math.round(evExpenses)}€</strong>
                      </div>
                    )}
                  </div>
                )
              ...
              {/* Total */}
              <div className="card" style={{ padding: '14px 16px', background: `${BASE.bgHover}20` }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: BASE.textMuted, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>
                  Bilan consolidé
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, textAlign: 'center' }}>
                  <div>
                    <div style={{ fontSize: 18, fontWeight: 600, color: SEMANTIC.success }}>{Math.round(revenueData.caReel + salesTotals.total)}€</div>
                    <div style={{ fontSize: 9, color: BASE.textMuted }}>Total revenus</div>
                  </div>
                  <div>
                    <div style={{ fontSize: 18, fontWeight: 600, color: SEMANTIC.danger }}>{Math.round(expenseData.total)}€</div>
                    <div style={{ fontSize: 9, color: BASE.textMuted }}>Total dépenses</div>
                  </div>
                  <div>
                    <div style={{ fontSize: 18, fontWeight: 600, color: margin >= 0 ? SEMANTIC.success : SEMANTIC.danger }}>{Math.round(margin)}€</div>
                    <div style={{ fontSize: 9, color: BASE.textMuted }}>Marge nette</div>
                  </div>
                </div>
              </div>
            </div>
      ...
      {section === 'depreciation' && (
        <div>
          {depreciationItems.length === 0 ? (
            <div className="empty-state" style={{ padding: 32 }}>
              <div className="empty-icon">{createElement(TrendingDown, { size: 40, color: BASE.textMuted })}</div>
              <div className="empty-text">Aucune immobilisation</div>
              <div style={{ fontSize: 11, color: BASE.textDisabled, marginTop: 4 }}>
                  ...
                  <div key={i} className="card" style={{ padding: '12px 14px' }}>
                    ...
                    <div style={{ height: 4, borderRadius: 2, background: BASE.bgHover, overflow: 'hidden' }}>
                      <div style={{
                        ...
                        background: pct >= 100 ? SEMANTIC.success : SEMANTIC.warning,
...
function AddExpenseForm({ events, orgId, onDone, onToast, cats }) {
  ...
  return (
    <div className="card" style={{ padding: 16, marginBottom: 14 }}>
      <div style={{ fontSize: 13, fontWeight: 600, color: BASE.text, marginBottom: 12 }}>Nouvelle dépense</div>
...
function KpiBox({ label, value, color }) {
  return (
    <div style={{
      ...
      background: BASE.bgHover, borderRadius: 10, border: `1px solid ${BASE.bgHover}`,
    }}>
