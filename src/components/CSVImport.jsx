    ...
    <Modal title="Importer des produits (CSV)" onClose={onClose}>
      {!rows.length ? (
        <div style={{ textAlign: 'center', padding: '20px 0' }}>
          ...
          <p style={{ fontSize: 13, color: '#9A8B94', marginBottom: 16, lineHeight: 1.5 }}>
          ...
          <div style={{ marginTop: 20, padding: '14px', borderRadius: 12, background: '#F0E8E4', textAlign: 'left' }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: '#3D3042', marginBottom: 6 }}>Format attendu :</div>
            <code style={{ fontSize: 10, color: '#9A8B94', lineHeight: 1.8, display: 'block' }}>
        ...
        <div style={{ textAlign: 'center', padding: '20px 0' }}>
          ...
          <div style={{ fontSize: 16, fontWeight: 800, color: '#3D3042', marginBottom: 8 }}>Import terminé</div>
        ...
        <div>
          <div style={{ fontSize: 13, fontWeight: 700, color: '#3D3042', marginBottom: 4 }}>
            {rows.length} ligne{rows.length > 1 ? 's' : ''} détectée{rows.length > 1 ? 's' : ''}
          </div>
          <div style={{ fontSize: 11, color: '#9A8B94', marginBottom: 16 }}>
            Colonnes : {headers.join(', ')}
          </div>

          {/* Field mapping */}
          <div style={{ fontSize: 12, fontWeight: 800, color: '#3D3042', marginBottom: 8 }}>
            Correspondance des colonnes
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 20 }}>
            {FIELDS.map(f => (
              <div key={f.key} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: 11, fontWeight: 600, color: '#9A8B94', width: 120, flexShrink: 0 }}>
                  {f.label}
                </span>
                <select
                  className="input"
          ...
          {/* Preview */}
          <div style={{ fontSize: 11, fontWeight: 700, color: '#9A8B94', marginBottom: 6 }}>Aperçu (3 premières lignes)</div>
          <div style={{ overflowX: 'auto', marginBottom: 16 }}>
            <table style={{ fontSize: 10, borderCollapse: 'collapse', width: '100%' }}>
              <thead>
                <tr>{headers.map(h => <th key={h} style={{ padding: '4px 6px', textAlign: 'left', borderBottom: '1px solid #E8DED8', color: '#9A8B94', fontWeight: 700 }}>{h}</th>)}</tr>
              </thead>
              <tbody>
                {rows.slice(0, 3).map((r, i) => (
                  <tr key={i}>{headers.map(h => <td key={h} style={{ padding: '4px 6px', borderBottom: '1px solid #F0E8E4', color: '#3D3042' }}>{r[h]}</td>)}</tr>
                ))}
              </tbody>
            </table>
          </div>
