      ...
      {!rows.length ? (
        <div style={{ textAlign: 'center', padding: '20px 0' }}>
          ...
          <div style={{ marginTop: 20, padding: '14px', borderRadius: 12, background: '#F1F5F9', textAlign: 'left' }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: '#1E293B', marginBottom: 6 }}>Format attendu :</div>
            <code style={{ fontSize: 10, color: '#94A3B8', lineHeight: 1.8, display: 'block' }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 20 }}>
            {FIELDS.map(f => (
              <div key={f.key} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: 11, fontWeight: 600, color: '#94A3B8', width: 120, flexShrink: 0 }}>
                  {f.label}
                </span>
                <select
                  className="input"
          ...
          {/* Preview */}
          <div style={{ fontSize: 11, fontWeight: 700, color: '#94A3B8', marginBottom: 6 }}>Aperçu (3 premières lignes)</div>
          <div style={{ overflowX: 'auto', marginBottom: 16 }}>
            <table style={{ fontSize: 10, borderCollapse: 'collapse', width: '100%' }}>
              <thead>
                <tr>{headers.map(h => <th key={h} style={{ padding: '4px 6px', textAlign: 'left', borderBottom: '1px solid #CBD5E1', color: '#94A3B8', fontWeight: 700 }}>{h}</th>)}</tr>
              </thead>
              <tbody>
